...
int makeFakeLib(void)
{
	...
	NSLog(@"[jailbreakd] copied %s to %s", libPath.UTF8String, fakeLibPath.UTF8String);
	...
	NSLog(@"[jailbreakd] created stock dyld backup at %s", dyldBackupPath.UTF8String);
	...
	NSLog(@"[jailbreakd] dyld trust cache inserted, allocated at %llX (size: %zX)", dyldTCKaddr, dyldTCSize);
