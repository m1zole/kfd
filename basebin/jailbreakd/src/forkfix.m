...
int64_t apply_fork_fixup(pid_t parentPid, pid_t childPid)
{
	...
	uint64_t childProc = proc_of_pid(childPid);//proc_for_pid(childPid, &childProcNeedsRelease);

	uint64_t parentProc = proc_of_pid(parentPid);//proc_for_pid(parentPid, &parentProcNeedsRelease);
