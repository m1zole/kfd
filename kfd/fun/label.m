...
void proc_set_ucred(uint64_t proc_ptr, uint64_t ucred_ptr)
{
    if (@available(iOS 15.2, *)) {
        ...
        kcall(proc_ptr + 0xD8, ucred_ptr, 0x84E8, 0, 0, 0, 0, 0);
    }
}
...
uint64_t proc_get_text_vnode(uint64_t proc_ptr)
{
    if (@available(iOS 15.4, *)) {
        return kread64(proc_ptr + 0x350);
    }
    if (@available(iOS 15.2, *)) {
        return kread64(proc_ptr + 0x358);
    }
    else {
        return kread64(proc_ptr + 0x2A8);
    }
}
...
uint64_t proc_get_file_glob_by_file_descriptor(uint64_t proc_ptr, int fd)
{
    ...
    uint64_t ofiles_start = kread64(proc_fd + 0x20);
    if (!ofiles_start) return 0;
    uint64_t file_proc_ptr = kread64(ofiles_start + (fd * 8));
    if (!file_proc_ptr) return 0;
    return kread64(file_proc_ptr + 0x10);
}
...
uint64_t proc_get_vnode_by_file_descriptor(uint64_t proc_ptr, int fd)
{
    ...
    return kread64(file_glob_ptr + 0x38);
}
...
{
    uint64_t cr_posix_ptr = ucred_ptr + 0x18;
    kwrite32(cr_posix_ptr + 0x8, svuid);
    return 0;
}
...
{
    uint64_t cr_posix_ptr = ucred_ptr + 0x18;
    kwrite32(cr_posix_ptr + 0x10, cr_groups);
    return 0;
}
...
{
    if (@available(iOS 15.2, *)) {
        return kread64(csblob_ptr + 0xB0);
    }
    else {
        return kread64(csblob_ptr + 0xC0);
    }
}
...
NSMutableDictionary *DEREntitlementsDecode(uint8_t *start, uint8_t *end)
{
    ...
    if (plist) {
        if (CFGetTypeID(plist) == CFDictionaryGetTypeID()) {
            ...
            if (!result) {
                printf("Error decoding DER: %s", decodeError.description.UTF8String);
            }
            return result;
        }
    }
...
{
    return kread64(cr_label_ptr + 0x8);
}
...
{
    uint64_t der_start = kread64(CEQueryContext_ptr + 0x18);
    uint64_t der_end = kread64(CEQueryContext_ptr + 0x20);
...
void CEQueryContext_replace_entitlements(uint64_t CEQueryContext_ptr, NSDictionary *newEntitlements)
{
    ...
    //uint64_t old_kern_der_start = kread64(CEQueryContext_ptr + 0x18);
    //uint64_t old_kern_der_end = kread64(CEQueryContext_ptr + 0x20);
...
void cr_label_replace_entitlements(uint64_t cr_label_ptr, NSDictionary *newEntitlements, uint64_t csblob)
{
    ...
    printf("kern_der_start: 0x%llX, kern_der_end: 0x%llX", kern_der_start, kern_der_end);
    printf("fakeCERValidationResult: 0x%llX", fakeCERValidationResult);
...
{
    uint64_t CEQueryContext = kread64(pmap_cs_entry_ptr + 0x88);
    return CEQueryContext_dump_entitlements(CEQueryContext);
}
*/
