...
int
kpf_init_kernel(addr_t base, const char *filename)
{
    size_t rv;
    uint8_t buf[0x4000];
    uint8_t *vstr;
    ...
    if (filename == NULL) {
        uint64_t runnningksize = sizeof(buf);
        rv = sizeof(do_kread(base, buf, sizeof(buf)));
    ...
    if (filename == NULL) {
        ...
        rv = sizeof(do_kread(kerndumpbase, kernel, kernel_size));
        if (rv != kernel_size) {
            free(kernel);
            kernel = NULL;
            return -1;
...
{
    addr_t val;
    addr_t ref = find_strref("com.apple.private.process.suspend-resume.any", 1, 0);
    if (!ref) {
        return 0;
    }
    ...
    ref -= kerndumpbase;
    printf("0x%llx\n", ref + kerndumpbase);
    val = follow_call64(kernel, ref - 24);
    if (!val) {
        return 0;
    }
    return val + kerndumpbase;
...
{
    addr_t val;
    addr_t ref = find_strref("spec_vnops.c", 1, 0);
    if (!ref) {
        return 0;
    }
    ...
    tmpref += kerndumpbase;
    printf("0x%llx\n", tmpref);
    val = bof64(kernel, xnucore_base, tmpref - 0x10);
    if (!val) {
        return 0;
    }
    return val + kerndumpbase;
...
{
    addr_t val;
    addr_t ref = find_strref("%s\t  lr: 0x%016llx  fp: 0x%016llx\n", 0, 0);
    if (!ref) {
        return 0;
    }
    ...
    ref -= kerndumpbase;
    printf("0x%llx\n", ref + kerndumpbase);
    val = follow_call64(kernel, ref - 0x2c);
    if (!val) {
        return 0;
    }
    return val + kerndumpbase;
...
{
    addr_t ret;
    addr_t ref = find_strref("com.apple.MobileStorageMounter", 1, 0);
    if (!ref) {
        return 0;
    }
    
    ref -= kerndumpbase;
    ret = calc64(kernel, ref - 0x1c8, ref  - 0x16c, 8);
    if (!ret) {
        return 0;
    }
...
{
    addr_t ref = find_strref("%s: failed pmap_enter, virt=%p, start_addr=%p, end_addr=%p, prot=%#x, flags=%#x @%s:%d", 1, 0);
    if (!ref) {
        return 0;
    }
    ref -= kerndumpbase;
    printf("0x%llx", ref + kerndumpbase);
    return ref + kerndumpbase + 0xc;
}
...
uint64_t find_mac_label_set(void)
{
    ...
    ref -= kerndumpbase;
    printf("0x%llx\n", ref);
    val = xref64(kernel, ref - 48, ref - 10, 5);
    if (!val) {
        return 0;
    }
    return val + kerndumpbase;
...
{
    addr_t ret, val;
    addr_t ref = find_strref("%s: illegal VA: %p; virt base 0x%llx, size 0x%llx @%s:%d", 1, 0);
    ref -= kerndumpbase;
    val = calc64(kernel, ref - 0x60, ref - 0x44, 11);
    if (!val) {
        return 0;
    }
    printf("0x%llx\n", kerndumpbase);
